...
interface SomeObject {
    [a: string]: { cvalue: number | string | undefined | SomeObject } | undefined;
}
...
function summ(a: SomeObject) {
    const x: number[] = Object.keys(a).map((k) => {
        ...
        if (typeof elem?.cvalue !== 'number' && elem?.cvalue) return summ(elem?.cvalue);
        if (typeof elem?.cvalue === 'number' && elem?.cvalue) return elem?.cvalue;
        else return 2022
    });
    let sum = 0;
